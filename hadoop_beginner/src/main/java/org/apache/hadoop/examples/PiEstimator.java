 ...
 * method. Mapper: Generate points in a unit square and then count points
 * inside/outside of the inscribed circle of the square. Reducer: Accumulate
 * points inside/outside results from the mappers. Let numTotal = numInside +
 * numOutside. The fraction numInside/numTotal is a rational approximation of
 * the value (Area of the circle)/(Area of the square), where the area of the
 * inscribed circle is Pi/4 and the area of unit square is 1. Then, Pi is
 * estimated value to be 4(numInside/numTotal).
 */
public class PiEstimator extends Configured implements Tool
{
	...
	public static class PiMapper extends MapReduceBase implements Mapper<LongWritable, LongWritable, BooleanWritable, LongWritable>
	{
		 ...
		 * @param reporter
		 */
		public void map(LongWritable offset, LongWritable size, OutputCollector<BooleanWritable, LongWritable> out, Reporter reporter)
			throws IOException
		{
	 ...
	 * from the mappers.
	 */
	public static class PiReducer extends MapReduceBase implements Reducer<BooleanWritable, LongWritable, WritableComparable<?>, Writable>
	{
		 ...
		 * @param reporter
		 */
		public void reduce(BooleanWritable isInside, Iterator<LongWritable> values, OutputCollector<WritableComparable<?>, Writable> output, Reporter reporter)
			throws IOException
		{
			if (isInside.get())
			{
		...
		throws IOException
	{
		...
		try
		{
			...
			return BigDecimal.valueOf(4).setScale(20).multiply(BigDecimal.valueOf(numInside.get())).divide(BigDecimal.valueOf(numMaps))
