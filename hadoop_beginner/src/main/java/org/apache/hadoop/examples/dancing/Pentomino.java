...
public class Pentomino
{
	...
	/**
	 * This interface just is a marker for what types I expect to get back as
	 * column names.
	 */
	...
	/**
	 * Convert a solution to the puzzle returned by the model into a string that
	 * represents the placement of the pieces onto the board.
	 * 
	 * @param width
	...
	public static String stringifySolution(int width, int height, List<List<ColumnName>> solution)
	{
		...
		for (List<ColumnName> row : solution)
		{
			...
			// for each point where the piece was placed, mark it with the piece
			// name
			for (ColumnName item : row)
			{
	 ...
	 */
	private static class SolutionPrinter implements DancingLinks.SolutionAcceptor<ColumnName>
	{
		int width;
		int height;
	 ...
	 * offset and size of the piece? This only checks in one dimension.
	 * 
	 * @param offset
	 ...
	 * @param upperLeft
	 *            is the piece constrained to the upper left of the board? this
	 *            is used on a single piece to eliminate most of the trivial
			...
			for (int x = 0; x < width; ++x)
			{
				for (int y = 0; y < height; ++y)
				{
					if (y + shape.length <= height && x + shape[0].length <= width
						&& (!upperLeft || (isSide(x, shape[0].length, width) && isSide(y, shape.length, height))))
					{
	 ...
	 * 
	 * @param split
	 *            a list of row indexes that should be choosen for each row in
	 *            order
	 ...
	 * 
	 * @param printer
	 *            A call-back object that is given each solution as it is found.
	 */
