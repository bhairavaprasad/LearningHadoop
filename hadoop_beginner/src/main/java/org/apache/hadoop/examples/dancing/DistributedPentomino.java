 ...
 * prefixes of length N with each unique prefix as a separate line. A prefix is
 * a sequence of N integers that denote the index of the row that is choosen for
 * each column in order. Note that the next column is heuristically choosen by
 * the solver, so it is dependant on the previous choice. That file is given as
 * the input to map/reduce. The output key/value are the move prefix/solution as
 * Text/Text.
 */
public class DistributedPentomino extends Configured implements Tool
{
	...
	public static class PentMap extends MapReduceBase implements Mapper<WritableComparable, Text, Text, Text>
	{
		...
		/**
		 * For each solution, generate the prefix and a string representation of
		 * the solution. The solution starts with a newline, so that the output
		 * looks like: <prefix>, <solution>
		 */
		...
		/**
		 * Break the prefix string into moves (a sequence of integer row ids
		 * that will be selected for each column in order). Find all solutions
		 * with that prefix.
		 */
		...
		public void configure(JobConf conf)
		{
			...
			pent = (Pentomino) ReflectionUtils.newInstance(conf.getClass("pent.class", OneSidedPentomino.class), conf);
